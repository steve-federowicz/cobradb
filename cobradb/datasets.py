...
from ome.base import (Base, DataSource, Session)
...
class InVivoEnvironment(Environment):
    ...
    __table_args__ = (UniqueConstraint('carbon_source', 'nitrogen_source',
                                       'electron_acceptor', 'temperature', 'supplements'), {})
...
class GenomeData(Base):
    ...
    def all_data(self):
        return [x['value'] for x in
                query_genome_data([self.dataset_id],
...
class DiffExpData(GenomeData):
    ...
    __table_args__ = (ForeignKeyConstraint(['dataset_id', 'genome_region_id'],
                                           ['genome_data.dataset_id', 'genome_data.genome_region_id'],
                                            ondelete='CASCADE'),
...
class ChIPPeakData(GenomeData):
    ...
    def __repr__(self):
        return "ChIP Peak: %d-%d %5.2f %s" %
            (self.genome_region.leftpos, self.genome_region.rightpos, 
             self.value, self.peak_analysis.name)
